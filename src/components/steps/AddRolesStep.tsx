...
export const AddRolesStep: React.FC<AddRolesStepProps> = ({
  ...
  const departments = useMemo(() => {
    const depts = Array.from(new Set(availableRoles.map(role => role.department)));
    return depts.filter(dept => dept && dept.trim() !== '');
  }, [availableRoles]);

  const filteredRoles = useMemo(() => {
    return availableRoles
      .filter(role => !selectedRoles.some(selected => selected.id === role.id))
      .filter(role => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-gray-600">{filteredRoles.length} Role(s)</p>
            </div>
            
            <div className="mt-4 space-y-2">
              <div className="grid grid-cols-3 gap-4 pb-2 border-b text-sm font-medium text-gray-500">
                <span>Role</span>
              ...
              {filteredRoles.map(role => (
                <div key={role.id} className="grid grid-cols-3 gap-4 items-center py-2 border-b border-gray-100">
          ...
          <div>
            <div className="bg-blue-50 p-4 rounded-lg">
              <h3 className="font-semibold text-gray-900 mb-2">In Product Department</h3>
              <p className="text-sm text-gray-600">{selectedRoles.length} Role(s)</p>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {selectedRoles.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">You currently don't have any roles in the Department.</p>
                </div>
              )}
            </div>
          </div>
